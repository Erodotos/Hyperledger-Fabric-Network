...
func (sc *SmartContract) WriteBatch(ctx contractapi.TransactionContextInterface, batch string) error {
	/*
		var data DataBatch
		err = json.Unmarshal([]byte(batch), &data)
		if err != nil {
			return nil, errors.New("DataBatch unmarshal failed!") //err
		}
	*/
	return nil
}
